...
static void i2c_scanner() {
    ...
    printf("\n>> I2C scanning ..."  "\n");
    uint8_t count = FISHTIME;
    for (uint8_t i = 1; i < 127; i++) {
        // printf("0x%X%s",i,"\n");
        if (testConnection(i, scanTimeout) == ESP_OK) {
            printf( "- Device found at address: 0x%X%s", i, "\n");
            count = (count == 0) ? FISHTIME : (count-1);
        }
    }
    if (count == 0)
...
static void test_alpha_display() {
    ...
    while (1) {
      displaybuffer[0] = numbertable[(count%10000)/1000];
      displaybuffer[1] = numbertable[(count%1000)/100];
      displaybuffer[2] = numbertable[(count%100)/10];
      displaybuffer[3] = numbertable[(count%10)/1];
